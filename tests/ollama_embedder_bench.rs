...
//!
//! Tests use the multi-threaded runtime because `OllamaEmbedder::embed` blocks
//! via `block_in_place`, which panics on a current-thread scheduler.
...
fn time_embed(embedder: &OllamaEmbedder, texts: &[&str]) -> Duration {
...
async fn ollama_single_embed_under_50ms() {
    let Some(embedder) = try_warm_embedder() else {
        return;
    };
...
async fn ollama_cold_call_under_500ms() {
    ...
    let embedder = OllamaEmbedder::new(MODEL, BASE_URL, DIMENSION).expect("create OllamaEmbedder");
    let start = Instant::now();
    let result = embedder.embed(&[SAMPLE_TEXT]);
